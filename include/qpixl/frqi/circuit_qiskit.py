"""
FRQI (Flexible Representation of Quantum Images) Circuit Generation
...
def compressed_frqi_circuit(a, compression=0.0):
    ...
    # Qubit numbering: qubit 0 is color/intensity, qubits 1 to k are address qubits
    color_qubit = 0
    
    # Hadamard register - apply to address qubits (qubits 1 to k)
    ...
    while i < (1 << k):  # 2^k
        ...
        # Add RY gate to color qubit
        if a[i] != 0:
            circuit.ry(a[i], color_qubit)
        
        # Loop over sequence of consecutive zero angles
        while True:
            # Compute control qubit
            if i == (1 << k) - 1:
                ctrl = 1  # First address qubit
            else:
                gray_xor = gray_code(i) ^ gray_code(i + 1)
                # Verify exactly one bit is set
                assert bin(gray_xor).count('1') == 1
                # Count trailing zeros to get which bit changed
                ctrl_bit = count_trailing_zeros(gray_xor)
                # Add 1 because color qubit is at index 0
                ctrl = ctrl_bit + 1
        ...
        for j in range(k):
            if (pc >> j) & 1:
                circuit.cx(j + 1, color_qubit)
    
    # Reverse bits to match C++ big-endian convention
